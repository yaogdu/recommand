...
//					} 			
				...
				if(_this.currentPage == 1 ){//当前页为第一页
					$('<li><a href="javascript:void(0);">上一页</a></li>').attr('class',_this.pageNameCssName + ' ' + _this.currentPageNameCssName + ' prev').prependTo(pagebar);
				}
				else{
					//当前页大于第一页
					$('<li><a href="javascript:void(0);">上一页</a></li>').attr('class',_this.pageNameCssName + ' prev')
						 .bind("click", function(){_this.onClickPage(_this.currentPage-1);}).prependTo(pagebar);
				}
				 //处理Next
				if (_this.currentPage == _this.totalpage) {//当前页为最后一页 
					$('<li><a href="javascript:void(0);">下一页</a></li>').attr('class',_this.pageNameCssName + ' ' + _this.currentPageNameCssName + ' next').appendTo(pagebar);
				} else {   
					$('<li><a href="javascript:void(0);">下一页</a></li>').attr('class',_this.pageNameCssName + ' next')
